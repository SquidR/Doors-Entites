...
Spawner.runEntity = function(entityTable)
    ...
    for cycle = 1, math.max(math.random(cyclesConfig.Min, cyclesConfig.Max), 1) do
        local r = game.workspace.CurrentRooms[game.ReplicatedStorage.GameData.LatestRoom]
        local spawnnoclone = r.LightBase
        local spawnrange = r.LightBase:Clone()
        local x = spawnrange.Position.X + math.random(-spawnrange.Size.X/2,spawnrange.Size.X/2)
        local y = spawnrange.Position.Y + math.random(-spawnrange.Size.Y/2,spawnrange.Size.Y/2)
        local z = spawnrange.Position.Z + math.random(-spawnrange.Size.Z/2,spawnrange.Size.Z/2)
        spawnrange.Position = Vector3.new(spawnnoclone.Position.X,r.Base.Position.Y,spawnnoclone.Position.Z)
        entityModel.PrimaryPart.Position = Vector3.new(x, y, z)
    end
